...
uses
  ...
  Vcl.Imaging.jpeg, Vcl.Menus, sSkinManager, Vcl.MPlayer, ShellAPI;
...
procedure Tzag.FormCreate(Sender: TObject);
begin
var font:string;
begin
font:=extractfilepath(application.ExeName);
AddFontResource(PChar(font + 'шрифт\ofont.ru_Flyer Shadow.ttf'));
end;
end;

procedure Tzag.FormDestroy(Sender: TObject);
begin
var font:string;
begin
font:=extractfilepath(application.ExeName);
...
begin
Form7.Show;
Form7.WebBrowser1.Navigate(extractFilePath(paramstr(0))+'я€€.htm');
PlaySound('клик.wav', 0, SND_ASYNC);
end;
...
begin
Form8.Show;
Form8.WebBrowser1.Navigate(extractFilePath(paramstr(0))+'ѕравило.htm');
PlaySound('клик.wav', 0, SND_ASYNC);
end;
...
begin
MediaPlayer1.FileName := ExtractFilePath(Application.ExeName) + 'песн€.mp3'; // «амените '»м€‘айла' на им€ вашего музыкального файла
  MediaPlayer1.Open;
  MediaPlayer1.Play;
  PlaySound('клик.wav', 0, SND_ASYNC);
end;
