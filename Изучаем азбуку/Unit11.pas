...
begin
  ...
  if Pole[col,row] > 200 then
   ...
   begin
      ...
      Form11.Canvas.Brush.Color := clBtnFace;
      Form11.Canvas.Pen.Color := clBtnFace;
      Form11.Canvas.Font.Color := clBtnFace;
    end;
  ...
  if (Pole[col,row] > 100) and (Pole[col,row] < 200)
   then
      ...
      Form11.Canvas.CopyRect(dst,Pictures.Canvas,src);

      // установить цвет границы и цифры
      Form11.Canvas.Pen.Color := clBlack;
      Form11.Canvas.Font.Color := clBlack;
      Form11.Canvas.Brush.Style := bsClear;
  end;
  ...
  // отрисовать клетку
  Form11.Canvas.Rectangle(x,y,x+WK-2,y+HK-2);
...
begin
  ...
  Pictures.LoadFromFile('картинки');
  ...
  Form11.ClientHeight := H * HK;
  Form11.ClientWidth := W * WK;


  Form11.Timer1.Enabled := False;
  Form11.Timer1.Interval := 200;
  ...
  if count = 1 then begin
    ...
    if (open1.col = open2.col) and (open1.row = open2.row)
      ...
      if Pole[open1.col,open1.row] = Pole[open2.col,open2.row] then
        ...
        begin
          n := n+1;
          Form11.Timer1.Enabled := True; // запустить таймер
...
begin
  ...
  begin
    Form11.Canvas.Font.Name := 'Times New Roman';
    Form11.Canvas.Font.Size := 36;
    Form11.Canvas.Font.Color := clBlack;
    Form11.Canvas.TextOut(70,160,'Game Over!');
  end;
end;
